 ...
 */
class TDProject_Project_Controller_Task_Json_Template
    extends TDProject_Project_Controller_Task_Json {

	/**
	 * This method is automatically invoked by the controller and implements
	 * the functionality to register tasks of the template with the ID passed 
	...
	{
        try {
    	    // load the template ID to load the tasks for from the Request
    	    $id = $this->_getRequest()->getParameter(
    	        TDProject_Project_Controller_Util_WebRequestKeys::ID, 
    	        FILTER_VALIDATE_INT
    	    );
    	    // cast the ID to an Integer
    	    $templateId = TechDivision_Lang_Integer::valueOf(
    	        new TechDivision_Lang_String($id)
    	    );
    	    // load the DTO with the template data
    	    $dto = $this->_getDelegate()->getTemplateViewData($templateId);
    		// register the templates's tasks in the Request
